...
export const content: Content = {
  ...
  howItWorks: {
    ...
    steps: [
      ...
      },
      {
        ...
        sample: [
          'ACTIVE.md         1.5 KB        injected at session start',
          'PERSONA.md · PLAYBOOK.md · 280 recaps      on request only',
        ],
      },
    ],
    ...
    funnel: {
      ...
      body: 'The reason this does not eat your context window is that almost none of it is injected. Raw history accumulates, synthesis compresses it, and only the short brief rides along into every session. Below is a real fingerprint after 280 sessions.',
      tiers: [
        {
          label: 'Accumulated',
          note: 'Append-only. Never edited, never summarised away.',
          files: [
            { name: 'observations.jsonl', size: '103 KB' },
            { name: 'reinforcements.jsonl', size: '3.8 KB' },
            { name: 'sessions/', size: '280 files' },
          ],
        },
        {
          label: 'Synthesized',
          note: 'Rewritten from the raw log. Readable by you, not just the agent.',
          files: [
            { name: 'PERSONA.md', size: '34 KB' },
            { name: 'PLAYBOOK.md', size: '20 KB' },
            { name: 'lessons.jsonl', size: '2.0 KB' },
          ],
        },
        {
          label: 'Injected',
          note: 'The only thing that costs you tokens on every session.',
          files: [{ name: 'ACTIVE.md', size: '1.5 KB' }],
        },
      ],
      footnote: 'Roughly 160 KB of accumulated context, 1.5 KB of it loaded by default. The other 158 KB is not gone — it is indexed, and the agent can pull the exact piece it needs.',
    },
  },
  workContext: {
    eyebrow: 'Not just how you think',
    headline: 'It knows the shape of what you work on.',
    body: 'A fingerprint is not only style. Over enough sessions Zug learns your company, your role, your stack, and which part of the system owns which problem — so a question about a broken thing does not start with fifteen greps and a request for you to explain the architecture again.',
    knows: [
    ...
    without: [
      'you: why is checkout failing?',
      'agent: grep -r "checkout" .',
      'agent: which service owns this?',
      'you: explains the architecture. again.',
      'agent: grep -r "payment" .',
      '… 13 more greps',
    ],
    withZug: [
      'you: why is checkout failing?',
      'agent: payments-api owns checkout; the gateway only proxies',
      'agent: opens payments-api, reads the handler',
      'agent: last time this broke it was the retry budget',
      'you: answers the actual question',
    ],
    callout: {
      label: 'Clues, not the whole file',
      body: 'ACTIVE.md carries pointers, not contents — enough for the agent to know that something is known and where to find it. When it needs the detail it asks for the specific memory. You get the benefit of a large fingerprint without paying for it in every prompt.',
    },
  },
  recaps: {
  ...
  superpower: {
    ...
    headline: 'Every agent you spawn already knows you.',
    ...
    withZug: {
      ...
      lines: [
        ...
        'Subagent A   inherits the same fingerprint',
        'Subagent B   inherits the same fingerprint',
      ],
    },
    fanout: {
      headline: 'It survives the fan-out.',
      body: 'This is the part that is hard to get any other way. When you spawn subagents, each one normally starts blind — you are delegating to a stranger who happens to share your codebase. They inherit the fingerprint instead, so the agent you hand work to already knows what you would have told it.',
      root: 'your fingerprint',
